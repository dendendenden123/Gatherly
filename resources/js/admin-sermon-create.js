$(document).ready(() => {
    ...
    $videoInput.on('change', function () {
        ...
        $.ajax({
                ...
                if (xhr.upload) {
                    xhr.upload.addEventListener('progress', function (e) {
                        if (e.lengthComputable) {
                            ...
                            $('#upload-bar').css('width', percent + '%').text(percent + '%');
                        }
                    }, false);
                }
                return xhr;
            ...
            if (res && res.success && res.url) {
                ...
                Swal.fire('Success', 'Video uploaded successfully. The Video URL has been filled.', 'success');
            } else {
                Swal.fire('Error', (res && res.message) ? res.message : 'Upload failed.', 'error');
            }
        }).fail((xhr) => {
    ...
    $form.on('submit', function (e) {
        ...
        if (uploadInProgress) {
            e.preventDefault();
            Swal.fire('Please wait', 'Video is still uploading. Submit after it completes.', 'info');
            return false;
        }
        ...
        if (!$videoUrl.val()) {
            ...
            e.preventDefault();
            Swal.fire('Missing video', 'Please provide a Video URL or upload a video file.', 'warning');
            return false;
        }
